...
export function match<T, E>(
...
): MatchFunc<T, E | LambdaDBDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | LambdaDBDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new LambdaDBDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new LambdaDBDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
